...
// 設置 Auth Token
const setAuthToken = (token, expiryDays = 7) => {
  if (token) {
    // 計算過期時間
    const expired = new Date();
    expired.setTime(expired.getTime() + expiryDays * 24 * 60 * 60 * 1000);
    
    // 設置 Cookie
    document.cookie = `worldWearToken=${token};expires=${expired.toUTCString()};path=/;SameSite=Strict;Secure`;
...
// 設置用戶 ID Cookie
const setUserIdCookie = (userId, expiryDays = 7) => {
  if (userId) {
    const expired = new Date();
    expired.setTime(expired.getTime() + expiryDays * 24 * 60 * 60 * 1000);
    document.cookie = `worldWearUserId=${userId};expires=${expired.toUTCString()};path=/;SameSite=Strict;Secure`;
    return true;
  }
  return false;
};
...
export const loginUser = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      // 生成 JWT Token
      const rawToken = generateJWTToken(user);
      const formattedToken = `Bearer ${rawToken}`;
      
      // 設置 Token，有效期根據 "記住我" 設置
      const cookieExpireDays = userData.rememberMe ? 30 : 1;
      setAuthToken(formattedToken, cookieExpireDays);
      
      // 設置用戶 ID
      setUserIdCookie(user.id, cookieExpireDays);
      ...
      console.log("Token 生成成功:", formattedToken);
...
export const checkEmailExists = createAsyncThunk(
  ...
  async (email, { rejectWithValue }) => {
    try {
      ...
      // 檢查是否存在該郵件
      const exists = dbUsers.some(user => user.email === email);
...
export const signupUser = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      // 檢查電子郵件是否已存在
      const emailExists = dbUsers.some(u => u.email === userData.email);
...
export const fetchCurrentUser = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(error.message || '獲取用戶信息失敗');
    }
  }
);
...
export const logoutUser = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      // 清除用戶 ID Cookie
      document.cookie = "worldWearUserId=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/";
...
export const requestPasswordReset = createAsyncThunk(
  ...
  async (email, { rejectWithValue }) => {
    try {
      ...
      if (!user) {
        return rejectWithValue('查無此電子郵件');
      }
      ...
      return rejectWithValue('重設密碼請求處理過程中發生錯誤');
    }
  }
);
...
const authSlice = createSlice({
  ...
  initialState: {
    ...
    status: getAuthToken() ? 'logged-in' : 'idle',
    error: null,
...
export const loginUser = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      // 設置完整的用戶認證信息
      setFullUserAuth(userToStore, token, null, userData.cookieExpireDays || 7);
    ...
    try {
      ...
      if (userIdFromCookie) {
        ...
        if (user) {
          ...
          // 設置完整的用戶認證信息
          setFullUserAuth(userToStore, token, null, 7);
...
export const initializeAuth = () => {
  ...
  if (userIdFromCookie && token) {
    console.log("發現有效的用戶 ID 和 token");
    return { 
...
const authSlice = createSlice({
  ...
  initialState: {
    ...
    status: (getStoredUser() || getUserIdFromCookie()) ? 'logged-in' : 'idle',
    error: null,
    resetPasswordRequestStatus: 'idle'
  },
  reducers: {
...
//       // 使用 cookieUtils 中的方法設置完整的用戶認證信息
//       setFullUserAuth(userToStore, mockToken, mockRefreshToken);
...
//       // 重新設置用戶認證信息
//       setFullUserAuth(userToStore, tokenFromCookie, null);
...
// // 其他功能如 signupUser, requestPasswordReset 保持不變...
...
//     token: null,
//     refreshToken: null,
//     status: isUserLoggedInByCookie() ? 'logged-in' : 'idle',
//     error: null,
//     resetPasswordRequestStatus: 'idle'
//   },
//   reducers: {
...
//         state.user = action.payload;
//         state.status = 'logged-in';
//         state.token = getWorldWearTokenFromCookie();
//       })
//       .addCase(fetchCurrentUser.rejected, (state, action) => {
//         if (action.payload === '未登入') {
//           state.status = 'idle';
