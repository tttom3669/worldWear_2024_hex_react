  ...
  async (userData, { rejectWithValue }) => {
    try {
      ...
      if (response.data && response.data.accessToken) {
        ...
        // 儲存登入資訊
        cookieUtils.saveLoginInfo(userToStore, accessToken, cookieExpireDays);
  ...
  async (email, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data?.message || "檢查郵件是否存在時發生錯誤"
      );
    }
  }
);
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const response = await axios.get(
        `${API_PATH}/users/${userId}?_expand=favorites&_expand=orders&_expand=carts`
      );
