...
import { loginUser, signupUser, clearError, requestPasswordReset, checkEmailExists } from "../slice/authSlice";
...
export default function Login() {
  ...
  const emailCheckStatus = useSelector((state) => state.authSlice.emailCheckStatus);
  ...
  const registerForm = useForm({
    ...
    mode: "onChange" // 設置為 onChange 模式，輸入時即時驗證
  });
    ...
    const subscription = registerForm.watch((values, { name }) => {
      ...
      if (name === 'email') {
        ...
        if (email && registerForm.getFieldState('email').valid) {
          // 設置防抖動計時器，避免頻繁API請求，但使用很短的延遲讓用戶感覺即時
  ...
  const handleLogin = useCallback(async (data) => {
    try {
      ...
      console.log('登入成功，獲得 token:', loginResult.accessToken);
  ...
  }, [dispatch, toastAlert, registerForm, emailExists, checkedEmail]);
  ...
  }, [dispatch, toastAlert, forgotPasswordForm, initialForgotPasswordData]);
  ...
  }, [activeTab, dispatch, loginForm, registerForm, forgotPasswordForm, initialLoginData, initialRegisterData, initialForgotPasswordData]);
  ...
  useEffect(() => {
    if (authStatus === "signup-success" && isRegisterAttemptRef.current) {
      ...
      // 自動填入登入表單的電子郵件
      const registeredEmail = registerForm.getValues().email;
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="d-grid">
          <button 
            type="submit" 
            className="btn btn-primary"
            disabled={isLoading || isEmailChecking || errors.email?.message?.includes("已被註冊")}
          >
            {isLoading ? "註冊中..." : "註冊"}
          </button>
        </div>
