...
export default function Login() {
    ...
    if (validateLoginForm()) {
      ...
      try {
        ...
        document.cookie = `worldWearToken=${accessToken};${
          loginData.rememberMe ? ' max-age=2592000;' : ''
        }`;
        ...
        const errorMsg =
          error.response?.data?.message || '登入失敗，請檢查您的憑據';
  ...
  const handleReduxLogin = useCallback(
    async (e) => {
      e.preventDefault();
      if (validateLoginForm()) {
        setSubmitError(null);
        ...
        try {
          ...
          const errorMsg =
            typeof error === 'string' ? error : '登入失敗，請檢查您的憑據';
    ...
    async (e) => {
      ...
      if (validateForgotPasswordForm()) {
        try {
          ...
          await dispatch(
            requestPasswordReset(forgotPasswordData.email)
          ).unwrap();
  ...
  }, [authStatus, toastAlert, registerData.email, initialRegisterData]);
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="mb-3">
          ...
          <input
            ...
            type="email"
            className={`form-control bg-white ${
              formErrors.forgotEmail ? 'is-invalid' : ''
            }`}
          ...
          {formErrors.forgotEmail && (
            <div className="invalid-feedback">{formErrors.forgotEmail}</div>
          )}
        </div>
