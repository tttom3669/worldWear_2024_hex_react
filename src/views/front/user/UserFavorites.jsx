...
// import UserAside from "../../../components/front/UserAside";
// import useSwal from '../../../hooks/useSwal';
// import { getFavorites, removeFromFavorites, updateFavoriteItemQuantity, addFavoriteToCart } from "../../../slice/favoritesSlice";
...
//   const favoritesError = useSelector(state => state.favorites?.error || null);
//   const currentUser = useSelector(state => state.authSlice?.user || null);
...
//   // 檢查用戶登入狀態函數 - 使用 localStorage 而非 cookie
//   const checkUserLogin = () => {
//     // 檢查 Redux store 中的用戶信息
//     if (!currentUser) {
//       // 檢查 localStorage 中的 token 和 user 作為備用
...
//       if (typeof error === 'string' && (error.includes('請先登入') || error.includes('未登入'))) {
//         toastAlert({ icon: 'warning', title: '請先登入' });
//         navigate("/login");
//       } else {
//         toastAlert({ icon: 'error', title: '加入購物車失敗，請稍後再試' });
...
//   useEffect(() => {
//     if (currentUser) {
//       console.log("用戶信息變化，重新獲取收藏列表");
//       dispatch(getFavorites());
//     }
//   }, [currentUser, dispatch]);

//   // 準備要顯示的收藏數據 - 如果用戶沒有收藏，則使用預設資料
//   const displayItems = favoritesData.length > 0 ? favoritesData : defaultItems;
