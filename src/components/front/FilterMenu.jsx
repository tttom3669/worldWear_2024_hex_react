...
const generateCategoryMappingsFromProducts = (products) => {
  ...
  products.forEach(product => {
    ...
    // 添加類別項目，如果不存在的話
    if (categoryItem && !categoryMappings[classType][category].includes(categoryItem)) {
...
const generateCategoriesData = (products, currentCategory, categoryMappings) => {
  // 確定當前選擇的性別分類
  const selectedGender = currentCategory === 'men' ? '男裝' : 
                         currentCategory === 'women' ? '女裝' : null;
...
// CategoryItem 組件，使用 React.memo 進行性能優化
...
// FilterMenu 組件
export const FilterMenu = React.memo(({ isOffcanvas = false }) => {
...
export const FilterSortButton = React.memo(
...
export const SortFilter = React.memo(({ sortOption, handleSortChange }) => {
...
// // 動態生成類別數據的函數，支持更靈活的類別配置
// const generateCategoriesData = (products) => {
//   // 從產品中動態提取所有唯一的主類別和子類別
...
//   // 使用 useSelector 獲取狀態
//   const { filters: reduxFilters = {}, items: products = [] } = useSelector(
//     (state) => state.productsList
//   );

//   // 使用 useMemo 優化 categoriesData 的生成
//   const categoriesData = useMemo(
//     () => generateCategoriesData(products),
//     [products]
//   );
