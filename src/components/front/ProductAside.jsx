...
//     }
//   },
//   [currentGender, selectedSubCategories, dispatch, navigate, isOffcanvas, setActiveFilters, setProductStatusFilter]
// );
...
//     // 直接從 Redux 獲取原始 filters 數據和當前類別
//     const filters = useSelector((state) => state.productsList?.filters || {});
//     const currentCategory = useSelector((state) => state.productsList?.currentCategory || "");

//     // 從當前類別路徑中提取類別信息
//     const categoryInfo = useMemo(() => {
//       if (!currentCategory) return { gender: "", mainCategory: "", subCategories: [] };
...
//   //     dispatch(resetFilters());
//   //   },
//   //   [currentGender, selectedSubCategories, dispatch, navigate, isOffcanvas]
//   // );
//   // 處理細項類別按鈕點擊 - 支持多選
// // 處理細項類別按鈕點擊 - 支持多選並重置計數
// const handleSubCategoryClick = useCallback(
...
//   //   // 返回總計數
//   //   return filterCount + subcategoriesCount;
//   // }, [activeFilters, selectedSubCategories]);
//   // 定義通用的計算函數
//   // const calculateTotalFilterCount = useCallback((filters, subCategories) => {
//   //   // 計算篩選條件的數量
//   //   const filterCount = Object.values(filters || {}).reduce(
//   //     (count, labels) => {
//   //       if (!Array.isArray(labels)) return count;
//   //       return count + (labels.includes("全部") ? 0 : labels.length);
//   //     },
//   //     0
//   //   );

//   //   // 加上已選擇的細項類別數量
//   //   const subcategoriesCount = Array.isArray(subCategories) ? subCategories.length : 0;
...
//   const parts = setCurrentCategory.split("/");
//   // ...提取類別信息
// }, [setCurrentCategory]);
