...
const ProductAside = memo(({ isOffcanvas = false, toggleOffcanvas }) => {
  ...
  const handleClearSubCategories = useCallback(() => {
    if (selectedMainCategory) {
      ...
      const mainCategory = genderCategory?.categories.find(cat => cat.slug === selectedMainCategory);
  ...
  }, [selectedMainCategory, currentCategories, currentGender, dispatch, navigate, isOffcanvas]);
    ...
    if (lastSelectedCategoryPath) {
      ...
      if (pathParts.length >= 3) {
        ...
        if (pathParts.length >= 4) {
          ...
          // 更新 Redux 狀態
          dispatch(setCurrentCategory(`${gender}/${mainCategory}/${subcategories}`));
        } else {
          // 只有主類別
          ...
          <div key={gender.slug} className="category-wrap">
            <ul className="list-unstyled">
              {gender.categories.map((category, index) => {
                ...
                const isCurrentCategoryPath = selectedMainCategory === category.slug;
                        ...
                        {!isStatusCategory ? (
                          <div className="d-flex flex-wrap gap-2">
                            ...
                            {category.subCategories &&
                              category.subCategories.length > 0 && (
                                <button
                                  className={`btn ${
                                    isCurrentCategoryPath && selectedSubCategories.length === 0
                                      ? "btn-primary"
                          ...
                          <div className="d-flex flex-wrap gap-2">
                            ...
                            {category.subCategories &&
                              ...
                              category.subCategories.map((subCategory) => {
                                // 用資料庫中實際的值來判斷是否選中
                                const dbValue = STATUS_MAP[subCategory.slug] || subCategory.title;
                                const isSelected = productStatusFilter.includes(dbValue);
